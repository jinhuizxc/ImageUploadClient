...
public class SelectPicturePopupWindow extends PopupWindow implements View.OnClickListener {
    ...
    public void showPopupWindow(Activity activity) {
        ...
        popupWindow.showAtLocation(activity.getWindow().getDecorView(), Gravity.CENTER | Gravity.BOTTOM, 0, 0);//设置窗口位置
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.picture_selector_take_photo_btn://拍照
                if (null != mOnSelectedListener) {
                    mOnSelectedListener.OnSelected(v, 0);
                }
                break;
            case R.id.picture_selector_pick_picture_btn://本地选择
                if (null != mOnSelectedListener) {
                    mOnSelectedListener.OnSelected(v, 1);
                }
                break;
            case R.id.picture_selector_cancel_btn://取消按钮
                if (null != mOnSelectedListener) {
